 ...
 while True:
  if h >= 355 or h <= 10: #red
   if s >= 50:
    color = 1
    break
  elif h <= 140 and h >= 81: #green
   if s >= 50:
    color = 2
    break
  elif h >= 221 and h <= 240: #blue
   if s >= 50:
    color = 3
    break
  elif h <= 60 and h >= 51: #yellow
   if s >= 50:
    color = 4
    break
  elif h >= 170 and h <= 200: #cyan
   if s >= 50:
    color = 5
    break
...
def ascii_convert_fast(frame,output_x,output_y,left_blank,debug=False):
 ...
 if debug:
  ...
  screen += ' potential fps: ' + str(1 / t.times['ascii_convert_fast']) + ' output_x: ' + str(output_x) + ' output_y: ' + str(output_y) + ' COLOR: ' + str(COLOR)
 ...
 while True:
  ...
  if not skip_frame:
   ...
   if debug:
    ...
    for data in t.times:
     print(data + ': ' + "{0:.3f}s\t".format(t.times[data]),end='',flush=True)
    print('actual fps',"{0:.1f}".format(1/(t.times['actual frametime'])),'\tcurrent_frame',current_frame,'\tshould_be_at',should_be_at,'\tvideo progress %',(time_elapsed / duration_s) * 100,end='',flush=True)
...
# yt = YouTube(link)
 ...
 if path.isfile(filename + '.mp4'):
  ...
  yt.streams.filter(file_extension='mp4',progressive=True).order_by('resolution').desc().first().download(filename=filename)
  process_video(filename + '.mp4', fast_algo, debug)
  return
